...
export default function BuildingForRentDetailsForm({
  ...
  const tabOptions = useMemo(() => [t("listings.residential"), t("listings.commercial"), t("listings.residentialAndCommercial")], [t]);
  ...
  useEffect(() => {
    onFormDataChange?.([
      ...
      { type: "value", label: t("listings.landType"), value: tabOptions[tabIndex] ?? "" },
  ...
  return (
    <>
      ...
      <View style={{ marginBottom: hp(2) }}>
        <SegmentedControl variant="large" options={tabOptions} selectedIndex={tabIndex} onSelect={setTabIndex} />
      </View>
      <SliderWithInput label={t("listings.streetWidth")} value={streetWidth} onChangeValue={setStreetWidth} max={100} />
      <InlineWheelPickerField label={t("listings.apartments")} value={apartments} options={APARTMENT_PICKER_OPTIONS} modalTitle={t("listings.apartments")} onChangeValue={setApartments} />
      ...
      {["furnished","basement","water","electricity","drainageAvailability"].map((k) => (

...
export default function ApartmentForRentDetailsForm({ onFormDataChange }: CategoryFormProps): React.JSX.Element {
  ...
  const options = useMemo(() => [t("listings.single"), t("listings.family")], [t]);
  ...
  useEffect(() => {
    onFormDataChange?.([
      ...
      { type: "value", label: t("listings.category"), value: options[tabIndex] },
      ...
      { type: "toggle", label: t("listings.airConditioned"), enabled: toggles.airConditioned },
  ...
  return (
    <>
      ...
      <View style={{ marginBottom: hp(2) }}>
        <SegmentedControl variant="large" options={options} selectedIndex={tabIndex} onSelect={setTabIndex} />
      </View>
      ...
      <InlineWheelPickerField label={t("listings.floor")} value={floorValue || (floorOptions[0] ?? "")} options={floorOptions} modalTitle={t("listings.floor")} onChangeValue={setFloorValue} />
      ...
      {[
        ...
        <ToggleRow
          ...
          onValueChange={(v) => setToggles((prev) => ({ ...prev, [stateKey]: v }))}

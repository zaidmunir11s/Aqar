...
/**
 * Generate marked dates for period marking type (range with start/end circles)
 * @param startDateStr - Start date string
 * @param endDateStr - End date string
 * @returns Marked dates object for period marking
 */
export function generatePeriodMarkedDates(
  ...
  while (currentDate <= endDate) {
    ...
    newMarked[dateStr] = {
      startingDay: isStart,
      endingDay: isEnd,
      color: "#e5e7eb", // Light grey for the range
      textColor: "#111827",
      selected: isStart || isEnd,
      selectedColor: "#000000", // Black for start/end circles
      selectedTextColor: "#ffffff",
    };

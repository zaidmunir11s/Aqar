...
export const SearchRequestProvider = ({ children }: { children: ReactNode }) => {
  ...
  const loadRequests = useCallback(async () => {
    try {
      const res = await backendRequest<{ success: boolean; data: { requests: any[] } }>(
        "/search-requests"
      );
      const mapped: SearchRequestData[] = (res.data.requests ?? []).map((row: any) => ({
        id: String(row.id),
  ...
  const addRequest = useCallback(async (requestData: Omit<SearchRequestData, "id" | "createdAt">) => {
    try {
      ...
      const res = await backendRequest<{ success: boolean; data: { request: any } }>(
        "/search-requests",
  ...
  const deleteRequest = useCallback(async (id: string) => {
    try {
      await backendRequest("/search-requests/" + encodeURIComponent(id), { method: "DELETE" });
